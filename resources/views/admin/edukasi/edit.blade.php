...
<div class="flex min-h-screen bg-gray-50">
    ...
    <main class="container mx-auto p-6">
        <h1 class="text-2xl font-bold mb-6">Edit Materi Edukasi</h1>
        ...
        <form action="{{ route('admin.edukasi.update', $educationMaterial) }}" method="POST"
            ...
            <div class="mb-4">
                ...
                <input type="text" name="title" id="title" value="{{ old('title', $educationMaterial->title) }}"
            ...
            <div class="mb-4">
                ...
                <select name="type" id="typeSelect"
                    ...
                    <option value="article" {{ old('type', $educationMaterial->type) === 'article' ? 'selected' : ''
                        }}>Artikel</option>
                    <option value="video" {{ old('type', $educationMaterial->type) === 'video' ? 'selected' : ''
                        }}>Video</option>
                </select>
                @error('type')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
            ...
            <div class="mb-4" id="contentField">
                ...
                <textarea name="content" id="content" rows="6"
                    ...
                    placeholder="Tulis artikel di sini...">{{ old('content', $educationMaterial->content) }}</textarea>
                @error('content')
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4" id="urlField"
                style="display: {{ old('type', $educationMaterial->type) === 'video' ? 'block' : 'none' }};">
                <label for="url" class="block text-gray-700 font-semibold mb-2">URL Video</label>
                <input type="url" name="url" id="url" value="{{ old('url', $educationMaterial->url) }}"
            ...
            <div class="mb-4">
                <label for="thumbnail" class="block text-gray-700 font-semibold mb-2">Gambar Thumbnail Baru</label>
                ...
                <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
                @if($educationMaterial->thumbnail)
                <div class="mt-3">
                    <img src="{{ asset('storage/' . $educationMaterial->thumbnail) }}" alt="Thumbnail Saat Ini"
            ...
            <div class="mt-6 flex space-x-3">
                ...
                <a href="{{ route('edukasi.index') }}"
                    class="bg-gray-500 hover:bg-gray-600 text-white font-semibold px-6 py-2 rounded-lg transition duration-200 inline-block">
                    Batal
                </a>
            </div>
        </form>
    ...
    document.addEventListener('DOMContentLoaded', function () {
            ...
            if (select && contentField && urlField) {
                select.addEventListener('change', function () {
                    ...
                    // Don't hide content field regardless of type
